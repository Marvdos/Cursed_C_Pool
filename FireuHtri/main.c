...
void _t(int s)
{
    ...
    for(int i=0, b=1,bl=_(b),r=0;i<l;i++){if(i==bl){b+=1;bl=_(b);r+=(b%2?0:1);id-=(r+1);}for(int sp=0;sp<c/2-id;sp++)P(" ");for(int st=0;st<1+(id*2);star++)P("*");P("\n",id+=1);}
...
{
    int res=0;
    (ac==2)?_t(i_(av[1])?atoi(av[1]):0):P("Error, bad arguments.",res=84);
    return(res);
}